...
class jediPopMenuView: UIView {
    ...
    public var backColor = UIColor(white: 0, alpha: 1)
    /// init
    ///
    /// - Parameters:
    ...
    ///   - action: 回调方法
    init(width: CGFloat, height: CGFloat, point: CGPoint, items: [String], imgs: [UIImage],  action: ((Int) -> Void)?) {
        ...
        self.layerHeight = CGFloat(CGFloat(items.count) * itemHeight)
    ...
    private func drawMyTableView() {
        ...
        tableView.register(jediPopMenuViewCell.self, forCellReuseIdentifier: jediPopMenuViewCell.identifier)
    }
...
extension jediPopMenuView:UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier:  jediPopMenuViewCell.identifier) as? jediPopMenuViewCell else {
            return jediPopMenuViewCell()
        }
